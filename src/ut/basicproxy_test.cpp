...
TEST_F(BasicProxyTest, DuplicateCancel)
{
  // Tests CANCELing a request which is already mid-cancellation
  ...
  // Re-send the CANCEL before receiving any response.
  inject_msg(msg1.get_request(), tp);

  // This time we only see a 200 OK response.
  ASSERT_EQ(1, txdata_count());
