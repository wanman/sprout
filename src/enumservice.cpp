  ...
  try
  {
         ...
         ++nb_it)
    {
      try
      {
        ...
        NumberPrefix pfix;
        pfix.prefix = PJUtils::remove_visual_separators(prefix);

        if (parse_regex_replace(regex, pfix.match, pfix.replace))
        {
          // Create an array in order of entries in json file, and a map in 
          // order of key length
          new_number_prefixes.push_back(pfix);
          new_prefix_regex_map.insert(
                      std::make_pair(PJUtils::remove_visual_separators(prefix), 
                                     pfix));
